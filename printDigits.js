// Write a function called printDigits that takes in one argument, num. When given an integer, the function should print (console.log) each digit in reverse order, starting with the ones place. Do not do this by just turning the number into a string and reversing it. Solve the problem using math. (Hint: look up while loops, and don’t forget about the % operator.)

//For example, if you were given 1 you should simply print 1, if given 314 you should print 4, 1, 3, and if given 12 you should print 2, 1:

let rev = 0
let num = 1223489
let lastDigit


function printDigits(num){
    while (num != 0){
        lastDigit = num % 10 // Grabs last digit
        rev = rev * 10 + lastDigit // Adds last digit to reverse variable
        num = Math.floor(num/10) // Removes last digit from num
    }
    return rev
}

console.log(printDigits(num))

// While num is not equal to 0, lastDigit grabs the last digit of num. Rev multiplies the lastDigit by 10 and adds lastDigit, which adds the new lastDigit to our reverse number.
// Then the last digit on num is removed by dividing that number by 10 and removing the decimal (the last digit)